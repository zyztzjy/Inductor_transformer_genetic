...
def calculate_bandwidth(L_nH, N_t, N_b, W_t_um, W_b_um, G_c_um):
    ...
    if L_nH < 0.15:
        ...
        bw = 23.7 - 14.4 * (L_nH - 0.15) / 1.85

    turns_penalty = 1 - 0.15 * (N_t + N_b - 2)

    width_benefit = 1 + 0.03 * ((W_t_um + W_b_um) / 10 - 1)

    gap_effect = 1 + 0.02 * ((G_c_um - 60) / 40)

    asymmetry = abs(N_t - N_b)
    bw = bw * turns_penalty * width_benefit * gap_effect * (1 - 0.08 * asymmetry)
...
def calculate_q_factor(L_nH, N_t, N_b, W_t_um, W_b_um):
    if L_nH < 0.4:
        q = 3.2 + 11.6 * (L_nH / 0.4)
    elif L_nH < 1.2:
        q = 14.8 - 6.7 * (L_nH - 0.4) / 0.8
    else:
        q = 8.1 - 3.9 * (L_nH - 1.2) / 1.8


    turns_penalty = 1 - 0.08 * (N_t + N_b - 2)

    width_benefit = 1 + 0.03 * ((W_t_um + W_b_um) / 10 - 1)

    asymmetry_penalty = 1 - 0.08 * abs(N_t - N_b)

    q = q * turns_penalty * width_benefit * asymmetry_penalty
...
def calculate_insertion_loss(L_nH, N_t, N_b, W_t_um, W_b_um, G_c_um):


    base_il = 1.23 + 3.47 * (L_nH / 2.0)
    turns_loss = 0.24 * (N_t + N_b - 2)

    width_loss = 0.48 * (10 / (W_t_um + W_b_um) - 0.5)
    gap_loss = 0.19 * (60 / G_c_um)
    asymmetry_penalty = 1 + 0.05 * abs(N_t - N_b)

    il = (base_il + turns_loss + width_loss + gap_loss) * asymmetry_penalty
...
class InductorPredictor:
    ...
    def _neural_network_inference(self, N_t, N_b, W_t_um, W_b_um, G_c_um, D_i_um):
        ...
        try:
            ...
            L = calculate_inductance(N_t, N_b, W_t_um, W_b_um, G_c_um, D_i_um)
            bw = calculate_bandwidth(L, N_t, N_b, W_t_um, W_b_um, G_c_um)
            q = calculate_q_factor(L, N_t, N_b, W_t_um, W_b_um)
            il = calculate_insertion_loss(L, N_t, N_b, W_t_um, W_b_um, G_c_um)
            return L, bw, q, il
